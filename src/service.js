...
async function dialogflowGateway(msg) {
  ...
  return await axios
    .post(
      "http://localhost:5001/colombo-mail/us-central1/dialogflowGateway",
      article
    )
    .then((response) => {
      let options = [];
      if (response.data.fulfillmentMessages[1])
        if (
          response.data.fulfillmentMessages[1].payload.fields.options.listValue
        ) {
          console.log("listValue");
          let id = 1;
          response.data.fulfillmentMessages[1].payload.fields.options.listValue.values.forEach(
            (optionRaw) => {
              console.log("values");
              const newOption = {
